...
- The notebook expects the merged dataset to include `cik`, `revenue`, and either `fiscal_year` or `period`/`year` fields. After running the cleaning steps in the notebook, these will be present.
- The KPI functions now include defensive parsing and ordering to ensure correct growth and CAGR calculations.
- If you want CAGR over a specific window (e.g. last 3 years), the function can be extended; currently it uses the first and last available observation per `cik`.
